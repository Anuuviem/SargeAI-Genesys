...
if (isNil("KRON_UPS_INIT")) then {
	KRON_UPS_INIT=0;
  diag_log "[Sarge] Need to wait for init_UPSMON...";
};
...
while {_loop} do {
  ...
  if ( _isSoldier && !_exit) then {
    ...
    if (_pursue) then {		
      ...
      // We establish a flanking distance	
      _flankdist = ((random 0.5)+0.7)*KRON_UPS_safedist;
            
      
      // The flanking distance can not be greater than the distance of the target or we will be caught by the back
      ...
      // Calculate forward feed position			
      _frontPos = [_targetPos,_dir2, _flankdist] call MON_GetPos2D;	
            

      // Adapt the flanking angle in the distance		
      ...
      // Decide on the best flanking poin
      // We count the target positions of other more remote groups
      _fldest = 0;
      _fldest2 = 0;
      _fldestfront = 0;
      _i = 0;
      ...
      {
        if (isNil "_x") exitWith {}; //in theory should exit loop if there are no targets
        if (_foreachindex !=0 && {_i != _grpid} && {format["%1", _x] != "[0,0]"}) then { //Error Undefined variable in expression: _x
          //if (KRON_UPS_DEBUG>0) then {diag_log format["[Sarge][KRON_targetsPos] Testing vars in loop block! _i = %1, _grpid = %2, _x = %3",_i,_grpid,_x];};
          ...
          //if (KRON_UPS_DEBUG>0) then {diag_log format["[Sarge][KRON_targetsPos] Testing vars in loop block! _dist1 = %1, _dist2 = %2, _dist3 = %3",_dist1,_dist2,_dist3];};
      ...
      // Preference is the initial choice of address
      switch (_flankdir) do {
        case 1: 
          {_prov = 125};
        case 2: 
      ...
      // If it is positive means that there are more existing destinations far from the flanking position1, we take this primarily
      if (_fldest<_fldest2) then {_prov = _prov + 50;};
      if (_fldest2<_fldest) then {_prov = _prov - 50;};		

      // If the provability is negative it indicates that it will take the flank2 therefore the probability of taking 1 is 0
      if (_prov<0) then {_prov = 0;};
      
        
      // We evaluate the address based on the calculated probability
      ...
      // Position of default attack on the flank
      ...
      // If it is a reinforcement unit, it will always harass the enemy
...
//		if (!_nowp && {alive _npc} && {canmove _npc} && {_wptype == "MOVE"} && {_timeontarget >= 60}) then {
  ...
  if (_isplane || _inheli) then {
    ...
    if (isNil "_npc") exitWith { diag_log format["[Sarge][NPC] Exiting loop, no npc selected. NPC = %1",_npc];};
    
    // check external loop switch
    _cont = 1;
    _npcname = vehicleVarname _npc;
    _cont = (call compile format["KRON_UPS_%1",_npcname]); //this errors, going to have to add an isNil here too...
    
    if(SAR_DEBUG) then {diag_log format ["_cont = %1 for leader: %2", _cont, format ["KRON_UPS_%1",_npcname]];};
    if (isNil "_cont") exitWith {}; //isNil inserted here as well
...
// Clean global variables in this group
