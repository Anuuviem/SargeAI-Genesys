...
/*"MON_LOCAL_EXEC" addPublicVariableEventHandler { if (local ((_this select 1)select 0)) then {
  call ( compile format[(_this select 1)select 1,(_this select 1)select 0] ); //this seems to be broken and frankly, no idea what it does
};*/
...
KRON_UPS_flankAngle = 45; // Flanking angle
KRON_UPS_INIT = 0;        // Variable indicating that it has been initialized
...
KRON_targets0 =[];//objectives west
KRON_targets1 =[];//objectives east
KRON_targets2 =[];//resistence	
KRON_targetsPos =[];// Current destination positions.
KRON_NPCs = []; // Leaders of the current groups	
...
if (isNil("KRON_UPS_INIT") || KRON_UPS_INIT == 0) then {
  ...
  diag_log format["[Sarge][Testing] KRON_randomPos = %1",KRON_randomPos];
...
//Initialization done
KRON_UPS_INIT=1;	
diag_log format["[Sarge][Upsmon][Init][Final] End of script, we made it here! KRON_UPS_INIT = %1",KRON_UPS_INIT];
